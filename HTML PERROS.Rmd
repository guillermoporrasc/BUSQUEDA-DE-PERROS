---
title: "BUSQUEDA DE PERROS USANDO GOOGLE STREET VIEW"
output:
  html_document: default
  pdf_document: default
---

```{r setup,results='markup', message=FALSE, warning=FALSE,include=FALSE}
...
## RELACION DE PERROS/KM EN LAS LOCALIDADES

```{r, message=FALSE, warning=FALSE}
#PLOT DE PERROS POR KILOMETROS RECORRIDOS
  
PERROS_KILOMETROS<- read_excel("F:/BUSQUEDA PERROS/BUSQUEDA-DE-PERROS/DATOS BUSQUEDA PERROS.xlsx", sheet = "PERROS_KM")
...
Los puntos representan a cada zona de busqueda


## MAPA DE PERROS DUPLICADOS
...
Se observa que los perros duplicados se encuentran entre los limites de las zonas de busqueda

## RELACION DE PERROS DEAMBULANTES ENCONTRADOS EN ENCUESTAS Y BUSQUEDA POR STREET VIEW ##
...
PERROS_KM<- read_excel("F:/BUSQUEDA PERROS/BUSQUEDA-DE-PERROS/DATOS BUSQUEDA PERROS.xlsx", sheet = "PERROS_KM")
...
```{r}
  ...
  ggplot(data=PERROS_DEAMBULANTES,aes(x=ZONA, y=PERROS,color=METODO,size=KILOMETROS))+
     geom_point(alpha=0.8)+coord_flip()+labs(title = "DIFERENCIA ENTRE LOS METODOS DE BUSQUEDA")
```
...
ggplot(data=PERROS_DEAMBULANTES,aes(x=AREA, y=PERROS,color=METODO,size=KILOMETROS))+
  geom_point(alpha=0.7)+coord_flip()+labs(title = "PERROS EN RELACION AL AREA")
```


Los puntos representan a las zonas(localidades) donde se realizo la busqueda
